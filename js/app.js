...
function configurarEscuchadores() {
    ...
    // Filtros de reportes
    ElementosDOM.entradaBusqueda.addEventListener('input', renderizarTablaAsistencias);
    ElementosDOM.filtroTurno.addEventListener('change', renderizarTablaAsistencias);
    ElementosDOM.filtroFecha.addEventListener('change', renderizarTablaAsistencias);
    ElementosDOM.botonExportar.addEventListener('click', exportarExcel);

    // Búsqueda de empleados
    ElementosDOM.entradaBusquedaEmp.addEventListener('input', renderizarTablaEmpleados);

    // Modal de Empleados
    ElementosDOM.botonNuevoEmpleado.addEventListener('click', () => abrirModalEmpleado());
    ElementosDOM.modalCerrar.addEventListener('click', cerrarModalEmpleado);
    ElementosDOM.botonCancelarModal.addEventListener('click', cerrarModalEmpleado);
    ElementosDOM.modalFormulario.addEventListener('submit', manejarEnvioFormulario);
    ...
    ElementosDOM.zonaSubida.addEventListener('click', () => ElementosDOM.entradaArchivo.click());
    ElementosDOM.entradaArchivo.addEventListener('change', manejarSeleccionArchivo);
    ...
    window.api.obtenerDatos()
        .then(datos => {
            if (datos.success) {
                ...
                mostrarNotificacion('Error al cargar datos locales: ' + datos.error, 'danger');
            }
        })
        .catch(err => {
            console.error(err);
            mostrarNotificacion('Error al comunicarse con la base de datos de escritorio', 'danger');
        });
}

function recalcularAsistencia() {
    EstadoApp.resumenGlobal = procesarAsistencia(EstadoApp.marcas, EstadoApp.empleados);
...
function renderizarDashboard() {
    const matutino = new Set(
        EstadoApp.resumenGlobal.filter(r => r.turno === "MATUTINO").map(r => r.usuario)
    ).size;

    const vespertino = new Set(
        EstadoApp.resumenGlobal.filter(r => r.turno === "VESPERTINO").map(r => r.usuario)
    ).size;

    const nocturno = new Set(
        EstadoApp.resumenGlobal.filter(r => r.turno === "NOCTURNO").map(r => r.usuario)
    ).size;

    const total = Object.keys(EstadoApp.empleados).length;
...
function conectarDropdownCustom(dropdownId, triggerId, menuId, inputId, onChange) {
...
function renderizarTablaAsistencias() {
    ...
    datosFiltrados.forEach(r => {
        let badgeTurno = '';
        if (r.turno === 'MATUTINO') badgeTurno = '<span class="badge badge-primary">Matutino</span>';
        else if (r.turno === 'VESPERTINO') badgeTurno = '<span class="badge badge-warning">Vespertino</span>';
        else if (r.turno === 'NOCTURNO') badgeTurno = '<span class="badge badge-secondary">Nocturno</span>';
...
function renderizarTablaEmpleados() {
    ...
    const listaEmpleados = Object.entries(EstadoApp.empleados).map(([id, datos]) => {
        return { id, ...datos };
    });

    const filtrados = listaEmpleados.filter(e => {
        return e.id.toLowerCase().includes(filtroTexto) ||
    ...
    filtrados.forEach(e => {
        let badgeTurno = '';
        if (e.turno === 'MATUTINO') badgeTurno = '<span class="badge badge-primary">Matutino</span>';
        else if (e.turno === 'VESPERTINO') badgeTurno = '<span class="badge badge-warning">Vespertino</span>';
        else if (e.turno === 'NOCTURNO') badgeTurno = '<span class="badge badge-secondary">Nocturno</span>';
...
function actualizarUIModalTurno(valor) {
    const triggerText = document.getElementById('modalTurnoDropdownTrigger')?.querySelector('.selected-value');
...
function manejarEnvioFormulario(e) {
    ...
    window.api.guardarEmpleado(datosEmpleado)
        .then(data => {
            if (data.success) {
        ...
        .catch(err => {
            console.error(err);
            mostrarNotificacion('Error de comunicación con el sistema de archivos', 'danger');
        });
}
...
window.eliminarEmpleado = function (id) {
    ...
    window.api.eliminarEmpleado(id)
        .then(data => {
            if (data.success) {
...
function procesarArchivo(archivo) {
    ...
    lector.onload = function (evt) {
        ...
        if (registros.length === 0) {
            mostrarNotificacion('No se encontraron registros de marcas en el archivo', 'warning');
            return;
        }
...
function subirArchivo(registros) {
    window.api.cargarArchivo(registros)
        .then(data => {
            if (data.success) {
                if (data.added > 0) {
                    mostrarNotificacion(`Se importaron ${data.added} nuevas marcas de asistencia.`, 'success');
                } else {
                    mostrarNotificacion('El archivo no contenía marcas nuevas, todo está al día.', 'info');
                }
                ...
                mostrarNotificacion('Error al importar marcas: ' + data.error, 'danger');
            }
        })
        .catch(err => {
            console.error(err);
...
function exportarExcel() {
    ...
    const maximoValor = (arr, key) => arr.reduce((max, obj) => Math.max(max, obj[key] ? obj[key].toString().length : 0), key.length);
